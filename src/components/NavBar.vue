<template>
  <div class="bg-white">
    ...
    background-color: rgb(126 36 36 / var(--tw-bg-opacity));" class="text-white" v-if="loggedIn" @click="logout()" to="/logout">
          ...
          <router-link style="  --tw-bg-opacity: 1;
    background-color: rgb(126 36 36 / var(--tw-bg-opacity));" class="text-white" v-if="loggedIn"  to="/admin">
           <span>
           لوحه الادمن
           </span>
          </router-link>
    ...
    <div>
      <ul
        class="flex bg-gray-700 p-1  overflow-hidden flex-row-reverse text-lg text-white font-bold ulNav"
      style="background: #1c0a0a;"
        >
        <li>
          <router-link  to="/">
            <font-awesome-icon class="pl-1" :icon="['fas', 'home']" />
            <span class="px-2"> الصفحة الرئيسية </span>
          </router-link>
        </li>
        <li>
          <router-link to="/shop">
            <font-awesome-icon class="pl-1" :icon="['fas', 'cart-plus']" />
